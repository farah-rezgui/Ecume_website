...
exports.addToCart = async (req, res) => {
  ...
  if (produit) {
    ...
    cart.produit.push({ _id: id_produit, quantity: 1 });
  }
  await cart.save();
...
exports.getCart = async (req, res) => {
  ...
  const cart = await Cart.findById(id_cart).populate('produit');
  if (!cart) {
    return res.status(404).json({ message: "Cart not found" });
  }
  res.status(200).json(cart);
...
exports.deleteFromCart = async (req, res) => {
  ...
  const produitIndex = cart.produit.findIndex((prod) => prod._id.toString() === id_produit);
  if (produitIndex === -1) {
