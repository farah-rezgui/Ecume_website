...
module.exports.updateProduit = async (req, res) => {
  try {
    const { id } = req.params;
    let { titre, description, prix, quantityStock } = req.body;
    ...
    // Handle image update
    let image = checkIfProduitExists.image; // Keep existing image by default
    if (req.file) {
      // If a new file is uploaded, update the image path
      image = "/uploads/" + req.file.originalname;
    }

    const updateProduit = await produitModel.findByIdAndUpdate(
      id,
      {
        $set: {
          titre,
